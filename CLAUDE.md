...
- **リリースは全自動**: `VERSION` を上げて main にマージ → Tests 成功後に TestPyPI 公開、さらに GitHub Release（タグは `VERSION` そのもの、例 `1.0.8`）の自動作成 → 本番 PyPI 公開（`python-publish.yml`）。いずれも既存バージョンはスキップ（冪等）。手動の twine upload や Release 作成は不要
